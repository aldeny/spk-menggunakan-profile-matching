...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($id_siswa) || empty($ppdb) || empty($ipa) || empty($ips) || empty($mtk) || empty($bindo) || empty($psikotes) || empty($minat) || empty($minat_ortu)) {
        ...
        if ($konek->query($save_nilai_murni) === TRUE) {
            ...
            if ($minat == $psikotes) {
                $mnt = 5;
            } elseif ($minat == 'IPA' && $psikotes == 'IPA/IPS') {
                $mnt = 5;
            } elseif ($minat == 'IPA' && $psikotes == 'IPS/IPA') {
                $mnt = 2;
            } elseif ($minat == 'IPS' && $psikotes == 'IPA/IPS') {
                $mnt = 2;
            } elseif ($minat == 'IPS' && $psikotes == 'IPS/IPA') {
                $mnt = 5;
            } elseif ($minat != $psikotes) {
                $mnt = 2;
            }
